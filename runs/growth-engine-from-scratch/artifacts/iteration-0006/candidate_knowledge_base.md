...
Iteration: 14 component-order trace parity for Q2/Q4
...
For Q1-Q4, every trace row is read as input -> component -> state -> output and terminates in one direct file:line citation. Every domain now also carries a component-order table (`order | component | input | state | output | citation`) so architecture grounding is symmetrical across SEO/GEO, Content, Ads, and Social. The trace is intentionally component-granular so reviewers can check architecture grounding without reconstructing a prose paragraph. Shared Core is only cited where the step crosses the trust, approval, credential, schedule, ledger, or kill-switch boundary; domain repos own domain logic.
...
| order | component | input | state | output | citation |
|---:|---|---|---|---|---|
| 1 | campaign facts and style guide injection | campaign name, description, AI prompt template, placeholders | campaign/prompt-template state | ideation and outline constraints | `getuai-email-2.0.md:91-94` |
| 2 | recipient/persona import | CSV email, first_name, last_name, company, position | recipient records | personalization facts | `getuai-email-2.0.md:101-104` |
| 3 | ideation and outline contract | campaign facts, recipient facts, style variables | reusable prompt state | bounded content plan | `getuai-email-2.0.md:91-94` |
| 4 | draft generator | batch, recipient, prompt template | generated email rows | personalized draft output | `getuai-email-2.0.md:111-114` |
| 5 | edit/review gate | generated emails | approved content state | sendable batch | `getuai-email-2.0.md:111-118` |
| 6 | SMTP publisher | SMTP account, approved batch | tested account plus sent/failed status | publish outcome | `getuai-email-2.0.md:96-99` |
| 7 | post-publish learning store | sent/failed status, text/image/company artifacts | session-scoped artifacts and outcomes | learning input for later campaigns | `getuai-api.md:24-40` |
| 8 | retrieval/factual guard | factual question or grounded context | cited answer state | hallucination-controlled copy | `reddit-scount.md:233-239` |

Trace lock: Content is a gated outbound pipeline, not a generic writer. Style guide injection, recipient schema, SMTP testing, human review, and artifact/outcome storage are load-bearing because changing any of them breaks correctness or auditability. The LLM acts as planner/generator inside the campaign and recipient contract; humans approve generated content before publish; table UI, template file format, and frontend stack remain stylistic.
...
| order | component | input | state | output | citation |
|---:|---|---|---|---|---|
| 1 | listen seed analyzer | company URL | keywords, pain points, competitors, target subreddits | listen state | `reddit-scount.md:108-139` |
| 2 | topic and engagement discovery | analysis plus keyword/competitor index | posts/comments cache | engagement candidates | `reddit-scount.md:141-181` |
| 3 | monitor search adapter | query, maxResults | YouTube video result list | monitor candidates | `youtube-api-demo.md:48-54` |
| 4 | post/reply/search adapter | text, post_id, query, media | platform JSON and side effects | X post/reply/search output | `openclaw-marketing.md:7392-7422` |
| 5 | multi-channel engage gateway | channel message | routed session/inbox state | engage surface | `openclaw-marketing.md:132-158` |
| 6 | schedule and credit monitor | launchd schedule, Chrome session, threshold | heartbeat/alert state | rate-limit and credit accounting output | `x-api-credit-monitor.md:72-104` |
| 7 | moderation gate | generated reply, DM allowlist policy | allow/deny state | moderated automation | `openclaw-marketing.md:122-126` |

Trace lock: Social uses platform-native adapters behind a Gateway-style control plane. The corpus supports a shared engage surface and session/inbox routing, but not a fake universal social API: listen/topic selection remains Reddit-shaped, monitor can be YouTube-shaped, post/reply/search remains X-shaped, schedule and credit accounting are X-monitor-shaped, and generated actions must pass moderation before public or private delivery.
