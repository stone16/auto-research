...
Iteration: 16 restore Q3 Ads component table
...
Disagreement is architectural: `getuai-seo` converges on UI/MCP/AI product runtime (`getuai-seo.md:52-83`), `rankncompare` keeps a static publisher/content store, and `growth-engine-legacy` rejects engines owning platform facts (`growth-engine-legacy.md:45-49`). The recommendation is domain-owned crawl/evaluate/publish logic with shared Core identity, credentials, artifacts, approvals, schedules, ledgers, and kill-switch. Speed versus auditability is the trade-off: direct publishers ship quickly (`rankncompare.md:53-56`), but the from-scratch design should keep crawler, evaluator, and publisher logic domain-specific while Core owns trust, approval, and stop-state auditability (`growth-engine-legacy.md:43-50`, `growth-engine-legacy.md:83-88`).
...
ID lock: `q15.fm1`-`q15.fm9` are canonical. Every prophylactic should cite the relevant `q13.f*` foundation plus the cognition row that prevents misapplication. Worked integration example: `q15.fm6` uses `q12.cog1` to stop a universal social adapter from bypassing per-platform moderation, and its structural prevention is `q13.f7` human-in-loop control plus `q13.f2` credential leasing (`openclaw-marketing.md:122-158`, `growth-engine-legacy.md:83-88`).
