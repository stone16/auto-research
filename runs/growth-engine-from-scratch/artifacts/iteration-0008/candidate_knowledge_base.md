...
Iteration: 14 complete Q13 dependency wiring
...
| order | component | input | LLM role | human review point | output | load-bearing vs stylistic | citation |
|---:|---|---|---|---|---|---|---|
| 1 | campaign setup | campaign name, description, prompt template, style variables | none/orchestrator constraint | prompt approval before generation | content brief | load-bearing: campaign and style variables constrain every downstream draft | `getuai-email-2.0.md:70-94` |
| 2 | recipient import | email, first_name, last_name, company, position CSV | none | schema validation | recipient set | load-bearing: personalization fields are correctness input | `getuai-email-2.0.md:101-104` |
| 3 | ideation/outline contract | campaign prompt + recipient fields | generator/orchestrator | prompt edit before batch | outline/personalization plan | load-bearing: variables prevent free-form prompt drift | `getuai-email-2.0.md:91-94` |
| 4 | draft generator | batch, prompt, recipient | generator | generated rows visible before send | draft email rows | load-bearing: one draft per recipient | `getuai-email-2.0.md:111-114` |
| 5 | edit/review surface | generated emails | critic optional, human primary | explicit review before send | approved content | load-bearing: send is gated by review | `getuai-email-2.0.md:111-118` |
| 6 | publish/send adapter | SMTP account + approved batch | none | SMTP test and final send confirmation | sent/failed status | load-bearing: tested account and status persistence; stylistic: Excel-like UI | `getuai-email-2.0.md:96-118` |
| 7 | artifact/session store | images, text, company facts | none | form validation | durable session artifacts | load-bearing: shared storage links copy to facts | `getuai-api.md:24-40` |
| 8 | retrieval-grounded factual copy | query/context | generator with retrieval grounding | citation review | cited answer | load-bearing: citations reduce hallucination | `reddit-scount.md:233-239` |
...
| order | surface | component | input | platform parameterization | state | output | citation |
|---:|---|---|---|---|---|---|---|
| 1 | listen | Reddit opportunity analysis | company URL | subreddits, keywords, pain points, competitors | MySQL analysis records | listen map | `reddit-scount.md:108-139` |
| 2 | listen/topic selection | Reddit discovery | analysis + keyword/competitor index | keyword_index, competitor_index, limit | posts/comments cache | opportunity candidates | `reddit-scount.md:141-181` |
| 3 | monitor | YouTube search adapter | query, maxResults | video result count and query semantics | stateless/search response | video candidates | `youtube-api-demo.md:48-54` |
| 4 | post/reply/search | X skill | text, post_id, query, media | tweet length, reply id, xurl/media requirements | X side effects | platform JSON | `openclaw-marketing.md:7392-7422` |
| 5 | engage | Gateway multi-channel control plane | channel message/session | WhatsApp/Slack/Discord/etc. adapter semantics | routed session/inbox | engagement surface | `openclaw-marketing.md:132-158` |
| 6 | schedule/credit accounting | X launchd monitor | Chrome profile, account id, threshold | local time 09:00, X account, Lark webhook | heartbeat/logs | credit alert | `x-api-credit-monitor.md:72-104` |
| 7 | moderation | channel gating policy | generated reply/channel | dmPolicy, allowFrom, per-channel rules | allow/deny config | constrained automation | `openclaw-marketing.md:122-126` |

Unifying abstraction: the corpus supports a Gateway/inbox/control-plane abstraction for routing sessions and channel messages, but platform semantics remain per-adapter. The correct shared layer is session routing, quota visibility, moderation policy, and ledgers; listen/search/reply payload semantics remain isolated by platform.
...
Cognition synthesis: the recurring practitioner move is trigger discipline. Use a model only when its trigger is present, attach a worked/failed evidence pair, and route the resulting control to the matching skill or failure mode. That makes Q13-Q15 mechanical: shared infrastructure owns trust, schedules, ledgers, credentials, observability, LLM routing, approval, and kill-switches; domain cognition owns interpretation of ranking, tone, spend, and community context.

## Q13 - Shared Foundations

Decision rule: share tenant trust, credentials, schedules, ledgers, observability, LLM routing, and approval because these create cross-domain auditability and safety. Isolate schemas, external APIs, ranking logic, tone/register, community rules, campaign mutations, and domain kill criteria because those encode domain-specific semantics.

| row_id | foundation | shared contract | corpus evidence | domain-isolated boundary | hooks_to_q14_q15 |
|---|---|---|---|---|---|
| q13.f1 | identity/session | tenant/user/session context; Core signs or constructs RequestContext; engines never receive raw browser tokens | `growth-engine-legacy.md:43-50`, `getuai-api.md:23-37`, `getuai-ui.md:35-60` | domain repos may read scoped context but not own auth truth | `q14.m1`, `q14.m3`, `q14.m4`, `q15.fm1`, `q15.fm5` |
| q13.f2 | secrets/credentials | scoped credential leases, SDK/config lookup, never raw durable secrets in engines | `growth-engine-legacy.md:64-88`, `getuai-ads.md:21-28`, `x-api-credit-monitor.md:12-18`, `getuai-ads-sdk.md:146-161` | Google Ads, X, SMTP, Search, LLM adapters keep platform payload semantics | `q14.m1`, `q14.m2`, `q14.m4`, `q15.fm1`, `q15.fm4` |
| q13.f3 | data/artifacts/lake | session artifacts, growth artifacts, events/leads/scores, publishable content records | `getuai-api.md:24-42`, `rankncompare.md:49-56`, `attribution_v2.md:13-16` | per-domain schemas for products, emails, campaigns, posts, ranks | `q14.m1`, `q14.m2`, `q14.m3`, `q14.m4`, `q15.fm3`, `q15.fm5`, `q15.fm8` |
| q13.f4 | schedules/queues | Core-owned schedules/poller or explicit launchd-equivalent monitor contract | `growth-engine-legacy.md:64-88`, `x-api-credit-monitor.md:82-93`, `attribution_v2.md:13-16` | job payload and cadence remain domain-owned | `q14.m5`, `q14.m6`, `q15.fm1`, `q15.fm4` |
| q13.f5 | observability | Sentry/Langfuse/run-event logs, structured request IDs, cost/rank/sentiment history | `growth-engine-legacy.md:64-70`, `LLMRush.md:19-23`, `x-api-credit-monitor.md:7-17` | domain metrics decide alert meaning and kill criteria | `q14.m2`, `q14.m4`, `q14.m5`, `q14.m6`, `q15.fm4`, `q15.fm7` |
| q13.f6 | LLM gateway | shared model/provider routing, prompt/cost controls, retrieval/citation guardrails | `getuai-seo.md:78-91`, `getuai-email-2.0.md:111-114`, `reddit-scount.md:233-239` | prompts, style guides, ranking queries, and per-domain critic rubrics stay isolated | `q14.m3`, `q14.m6`, `q15.fm6`, `q15.fm7` |
| q13.f7 | human-in-loop console | review, approval, override, action ledger, kill-switch for every external write | `growth-engine-legacy.md:83-88`, `lawyer_marketing.md:248-269`, `getuai-email-2.0.md:111-118` | domain-specific done criteria and rejection reasons stay in the lane | `q14.m1`, `q14.m2`, `q14.m3`, `q14.m4`, `q14.m5`, `q14.m6`, `q15.fm1`, `q15.fm6` |
| q13.f8 | repo governance/template | AGENTS/CLAUDE rules, CI, version drift checks, branch/PR conventions, no secret/run leakage | `optiminds-repo-template.md:9-63`, `optiminds-org-config.md:7-44`, `growth-engine.md:33-63` | product/business code remains in consuming repos | `q14.m1`, `q14.m6`, `q15.fm2`, `q15.fm9` |

## Q14 - Build Sequence

| row_id | milestone | scope | dependencies | done_criteria | next_trigger | deferrals |
|---|---|---|---|---|---|---|
| q14.m1 | Day-1 | Core tenant/session/artifact/source/approval skeleton for Q13 foundations and `q15.fm1`/`q15.fm3` prevention | `q13.f1`, `q13.f2`, `q13.f3`, `q13.f7`, `q13.f8` | target/session/artifact/action records persist; no run leakage | first durable tool | ads/social writes |
| q14.m2 | Week-1 | SEO/GEO read lane: Q1 crawl/search/keyword/sitemap via `seo.skill6`-`seo.skill9` | `q13.f2`, `q13.f3`, `q13.f5`, `q13.f7`, plus `q1.trace1`-`q1.trace8` | outputs stored, rank source declared, sitemap exists, rank/source artifact observable, sitemap publish requires approval | recurring/publish request | autopublish |
| q14.m3 | Week-2 | Content lane: Q2 templates/import/draft/SMTP/retrieval via `content.skill1`-`content.skill5` | `q13.f1`, `q13.f2`, `q13.f3`, `q13.f6`, `q13.f7`, plus Q2 and Q6 rows | variables validate; drafts generated; review before send; cited factual copy retained when retrieval is used | outbound metric need | rich media |
| q14.m4 | Week-4 | Ads read + attribution via `ads.skill7`/`ads.skill8`; no spend mutation | `q13.f1`, `q13.f2`, `q13.f3`, `q13.f5`, `q13.f7`, plus Q3 reporting/attribution rows | reports + conversion events visible; read-only analyst; budget recommendation stored in action ledger before any write | repeated budget recs | budget mutation/A-B |
| q14.m5 | Week-8 | Controlled writes across domains using Q3/Q4 approval and moderation guardrails | `q13.f2`, `q13.f3`, `q13.f4`, `q13.f5`, `q13.f7`, plus `q15.fm4`/`q15.fm6` | every write has ledger, envelope, monitor, approval, and rollback/stop reason | stable weekly outcomes | full optimizer |
| q14.m6 | Month-3 | OODA orchestration + industry packs, especially `q9.cog3` vertical expertise | `q13.f1`, `q13.f3`, `q13.f4`, `q13.f5`, `q13.f6`, `q13.f7`, `q13.f8`, plus all lanes | weekly observe-plan-approve-execute-review works; industry pack rules visible; governance drift checked | tenant/industry scale | Temporal/marketplace/autonomous spend |

Evidence shape: thin prototype `getuai-2.0.md:19-42`, MVP route hardening `getuai-mvp.md:9-76`, production refactor `attribution_v2.md:16-23`, vertical cases `lawyer_finder.md:11-16`, `cuilawgroup.md:10-27`, backend-skeleton-first `growth-engine.md:8-12`. Deferral logic is failure-informed: do not build autopublish before `q14.m2` has rank/source evidence, observability, and `q13.f7` approval; do not build budget mutation or A/B before `q14.m4` proves attribution and routes recommendations through the action ledger; do not build autonomous spend before `q14.m6` proves OODA review, industry packs, observability, and governance.

## Q15 - Failure Modes

| row_id | failure_mode | domains | count | cause | early symptom | prophylactic | evidence_pair |
|---|---|---|---:|---|---|---|---|
| q15.fm1 | engines own platform facts | SEO/Ads/Social | 3 | scattered identity/creds/schedules | raw tokens/own cron | `q13.f1` identity, `q13.f2` leases, `q13.f4` schedules, `q13.f7` action ledger | `growth-engine-legacy.md:43-50`, `growth-engine-legacy.md:83-88` |
| q15.fm2 | legacy scaffolding import | all | 4 | copying attempts | stale paths | greenfield contracts and `q13.f8` governance; `q9.cog3` vertical specificity | `growth-engine.md:69-152` |
| q15.fm3 | docs without runtime | all | 4 | design outruns core | no `core/` tree | Day-1 skeleton `q14.m1` with `q13.f3` artifacts and `q13.f8` governance | `growth-engine-legacy.md:16-22` |
| q15.fm4 | credential drift | SEO/Ads/Social | 3 | auth outside action layer | API/re-login errors | `q13.f2` leases + `q13.f5` monitors + `q13.f4` scheduled checks; `social.skill4` | `getuai-ads.md:24-28`, `x-api-credit-monitor.md:13-17` |
| q15.fm5 | attribution/session breakage | Ads/Content | 2 | cookie/user rotation | missing lead rows | `q13.f1` identity/session and `q13.f3` event/artifact model; SDK domain/session tests; `ads.skill8` | `attribution_v2.md:117-186` |
| q15.fm6 | writes without approval | SEO/Ads/Social | 3 | mixed read/write skills | agents mutate while analyzing | read-only diagnosis + `q13.f7` ledger/approval + `q13.f6` LLM guardrails; `q11.cog5`, `q12.cog1`, and `q12.cog5` guard public actions | `lawyer_marketing.md:248-269`, `growth-engine-legacy.md:84` |
| q15.fm7 | static publisher as GEO evaluator | SEO/GEO | 1 | indexing confused with LLM visibility | sitemap but no LLM rank | LLMRush sensor `content.skill6` with `q13.f5` observability and `q13.f6` LLM gateway | `rankncompare.md:134-149`, `LLMRush.md:7-14` |
...
| q15.fm9 | API prefix mismatch | shared | 2 | local/prod route drift | wrong backend path | `q13.f8` API_PREFIX/proxy contract and repo governance | `getuai-mvp.md:9-76` |

ID lock: `q15.fm1`-`q15.fm9` are the canonical failure-mode rows. When a benchmark answer claims a prophylactic, it should cite the relevant `q15.fm*` row plus the cognition row and `q13.f*` foundation that prevents misapplication; for example `q15.fm6` uses `q12.cog1` to stop a universal social adapter from bypassing per-platform moderation and `q13.f7` to keep writes behind approval.

## Benchmark Answers

Machine-readable benchmark answers are emitted in the top-level `benchmark_answers` JSON array. In-KB summary: Q1 and Q3 carry explicit component-by-component trace locks; Q2 and Q4 now carry symmetric component-order tables; Q5-Q8 retain 34 skill rows with stable IDs (`seo.skill1`-`social.skill8`); Q9-Q12 carry stable cognition row IDs (`q9.cog1`-`q12.cog5`) and worked/failed evidence pairs; Q13 carries stable foundation IDs (`q13.f1`-`q13.f8`); Q14 carries milestone IDs (`q14.m1`-`q14.m6`) with complete Q13 dependency enumeration; Q15 carries failure-mode IDs (`q15.fm1`-`q15.fm9`) with exact Q13 prophylactic hooks.
